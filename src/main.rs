...
async fn main() {
    ...
    // Log to both console AND file with filtering
    use tracing_subscriber::layer::SubscriberExt;
    use tracing_subscriber::util::SubscriberInitExt;
    use tracing_subscriber::EnvFilter;
    
    // Filter: INFO by default, WARN for sqlx, allow RUST_LOG override
    let filter = EnvFilter::try_from_default_env()
        .unwrap_or_else(|_| {
            EnvFilter::new("upman_backend=info,tower_http=info,axum=info,sqlx=warn,hyper=warn,reqwest=warn")
        });
